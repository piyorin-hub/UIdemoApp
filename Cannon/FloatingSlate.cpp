...
FloatingSlateButton::FloatingSlateButton(const XMVECTOR& position, const XMVECTOR& size, const XMVECTOR& color, const unsigned id, IFloatingSlateButtonCallback* pCallback, const wstring& buttonText) :
	FloatingSlateButton(position, size, color, id, pCallback, shared_ptr<Texture2D>(nullptr))
{
	SetText(buttonText);
}
		...
		if (m_pushDistance == 0.0f)
		{
			if (m_on)
			{
				for (auto callback : m_registeredCallbacks)
					callback->OnButtonUnPressed(this);
...
void FloatingSlateButton::UpdateTexture()
{
	...
	if (m_texture->IsRenderTarget())
	{
		...
		if (m_buttonTextw.has_value() && !m_buttonTextw.value().empty())
		{
			DrawCall::DrawText(m_buttonTextw.value(), layoutRect, m_fontSize, HorizontalAlignment::Center, VerticalAlignment::Middle, textColor);
		}
		else if (m_buttonText.has_value() && !m_buttonText.value().empty())
		{
			DrawCall::DrawText(m_buttonText.value(), layoutRect, m_fontSize, HorizontalAlignment::Center, VerticalAlignment::Middle, textColor);
...
void FloatingSlateButton::SetText(const std::wstring& buttonText)
{
	if (buttonText != m_buttonTextw)
	{
		m_buttonTextw = buttonText;
		m_textureNeedsUpdate = true;
	}
}
...
{
	return m_buttonText.value();
}

const std::wstring& FloatingSlateButton::GetTextw()
{
	return m_buttonTextw.value();
}
...
std::shared_ptr<FloatingSlateButton> FloatingSlate::GetButton(const std::wstring& buttonText)
{
	for (auto& button : m_buttons)
		if (button->GetTextw() == buttonText)
			return button;
