...
public:
	...
	virtual void OnButtonUnPressed(class FloatingSlateButton* pButton) = 0;
};
...
public:
	...
	FloatingSlateButton();
	FloatingSlateButton(const XMVECTOR& position, const XMVECTOR& size, const XMVECTOR& color, const unsigned id, IFloatingSlateButtonCallback* pCallback, const std::wstring& buttonText);
	...
	void SetText(const std::wstring& buttonText);
...
private:
	...
	std::optional<std::string> m_buttonText;
	std::optional<std::wstring> m_buttonTextw;
	float m_fontSize;
...
public:
	...
	std::shared_ptr<FloatingSlateButton> GetButton(const std::wstring& buttonText);
